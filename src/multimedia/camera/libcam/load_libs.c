...
static LoadAvformat *newAvformat(void)
{
    ...
    PrintError();
    pAvformat->m_av_dump_format = (uos_av_dump_format)dlsym(handle1, "av_dump_format");
    PrintError();
...
static LoadAvutil *newAvutil(void)
{
    ...
    Avutil->m_av_dict_get = (uos_av_dict_get)dlsym(handle5, "av_dict_get");
    PrintError();
    Avutil->m_av_strerror = (uos_av_strerror)dlsym(handle5, "av_strerror");
    PrintError();
