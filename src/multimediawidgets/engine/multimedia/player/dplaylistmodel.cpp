...
void DPlaylistModel::slotStateChanged()
{
    ...
    if (!e) return;
    qInfo() << "model" << "_userRequestingItem" << m_userRequestingItem << "state" << e->state();
    switch (e->state()) {
...
DPlaylistModel::DPlaylistModel(PlayerEngine *e)
    : m_engine(e)
{
...
void DPlaylistModel::tryPlayCurrent(bool next)
{
    ...
    if (pif.valid) {
        ...
        bool result = std::any_of(m_infos.begin(), m_infos.end(), [](const PlayItemInfo & info) {
            return info.valid;
        });
        if (result) {
            canPlay = true;
...
{
    if (count() == 0) return;
    qInfo() << "playmode" << m_playMode << "fromUser" << fromUser
            << "last" << m_last << "current" << m_current;
...
void DPlaylistModel::collectionJob(const QList<QUrl> &urls, QList<QUrl> &inputUrls)
{
    for (const auto &url : urls) {
        ...
        if (m_loadFile.contains(url))
            continue;
        if (!url.isValid() || indexOf(url) >= 0 || m_urlsInJob.contains(url.toLocalFile()))
            continue;
...
void DPlaylistModel::delayedAppendAsync(const QList<QUrl> &urls)
{
    ...
    if (QThread::idealThreadCount() > 1) {
        if (!m_getThumanbil) {
            ...
            connect(m_getThumanbil, &GetThumanbil::updateItem, this, &DPlaylistModel::onAsyncUpdate, Qt::BlockingQueuedConnection);
...
{
    auto p = std::find_if(m_infos.begin(), m_infos.end(), [&](const PlayItemInfo & pif) {
        return pif.url == url;
    });
