...
//        l->addWidget(_current);
    ...
    connect(&m_networkConfigMng, &QNetworkConfigurationManager::onlineStateChanged, this, &PlayerEngine::onlineStateChanged);
...
//    setLayout(l);
    ...
    connect(m_playlist, &DPlaylistModel::asyncAppendFinished, this,
...
bool PlayerEngine::showOpenGLWgt(QOpenGLWidget *pVideoWidget)
{
    ...
    #ifdef __x86_64__
        connect(m_current, &DPlayerBackend::elapsedChanged, [ = ]() {//更新opengl显示进度
...
void PlayerEngine::paintEvent(QPaintEvent *e)
{
    ...
    if (!DCompositeManager::get().composited() || utils::check_wayland_env()) {  // wayland下不会进入mainwindow的paintevent函数导致图标未绘制
...
qint64 PlayerEngine::elapsed() const
{
    ...
    qint64 nDuration = m_current->duration();        //因为文件信息的持续时间和MPV返回的持续有些差别，所以，我们使用文件返回的持续时间
...
{
    if (m_current)
        m_current->setVideoRotation(degree);
}

void PlayerEngine::changeSoundMode(DPlayerBackend::SoundMode sm)
{
    if (m_current)
        m_current->changeSoundMode(sm);
}
